...
class AzuriteTableRequest {
    constructor({
        ...
        this.payload.PartitionKey = this.partitionKey;
        this.payload.RowKey = this.rowKey;
        ...
        for (const token of filter.split(' ')) {
            ...
            if (!token.match(/\d+/) &&
                ...
                if (token === 'PartitionKey' || token === 'RowKey') {
                    transformedQuery += `item.${token.replace('PartitionKey', 'partitionKey').replace('RowKey', 'rowKey')} `;
                } else {
